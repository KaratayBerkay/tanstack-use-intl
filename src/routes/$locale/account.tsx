...
function AccountPage() {
  ...
  return (
    <main className="mx-auto max-w-3xl px-6 py-12">
      ...
      <h1 className="mt-6 mb-6 text-4xl font-bold tracking-tight text-(--sea-ink)">
        {t('settings.title')}
      </h1>
      ...
      <section className="mb-5 rounded-xl border border-gray-200 bg-gray-50 p-6">
        ...
        <div className="grid gap-3">
          <Field label={t('profile.name')} value="Ada Lovelace" id="field-name" />
          <Field label={t('profile.email')} value="ada@example.com" id="field-email" />
          <Field label={t('profile.avatar')} value="avatar.png" id="field-avatar" />
        </div>
      </section>
      ...
      <div className="mt-6 flex gap-4">
        <Link
          ...
          to="/$locale"
          params={{ locale }}
          className="text-indigo-500 underline transition hover:text-indigo-700"
        >
          → Home
        </Link>
        <Link
          id="account-nav-about"
          to="/$locale/about"
          params={{ locale }}
          className="text-indigo-500 underline transition hover:text-indigo-700"
        >
